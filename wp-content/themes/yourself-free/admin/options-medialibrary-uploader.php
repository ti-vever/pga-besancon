...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '', $_input_name = '', $_pattern = false ) {
		...
		$custom_class = $id;
			...
			$input_name = $option_name.'['.$_input_name.']';
			$custom_class = $_input_name;
		}
				
		if( $_pattern ){
			$pattern_button = '<input class="pattern_upload_button button" type="button" value="' . __( 'Set pattern', 'theme_textdomain' ) . '" />' . "\n";
		}
		
		if ( $value ) { $class = ' has-file'; }
		
		$output .= '<input id="' . $id . '" class="upload ' . $custom_class . $class . '" type="text" name="'.$input_name.'" value="' . $value . '" />' . "\n";
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'theme_textdomain' ) . '" rel="' . $int . '" />' . "\n";
		$output .= $pattern_button;
		...
		if ( $value != '' && 0 === preg_match('/^\d+\.png$/', $value) ) { 
		...
		}elseif( $value != '' && 1 === preg_match('/^\d+\.png$/', $value) ){
			$remove = '<a href="javascript:(void);" class="mlu_remove button">Remove</a>';
			$image = preg_match( '/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value );
			if ( $image ) {
				$output .= '<img src="' . get_template_directory_uri() . '/admin/images/patterns/' . $value . '" alt="" />'.$remove.'';
				$value . '" alt="" />'.$remove.'';
			}
		}
